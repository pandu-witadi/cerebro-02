...
print('/static/_next', BASE_DIR / "frontend/out/_next" )
print('/static', BASE_DIR / "frontend/out" )
...
@app.get("/api/ollama_model")
async def ollama_model():
    api_url = OLLAMA_URL + "/api/tags"
    async with httpx.AsyncClient() as client:
        response = await client.get(api_url)
        result = response.json()

        model_name = []
        for obj in result["models"] :
            model_name.append(obj["name"])
        return model_name


### WEBSOCKETS
async def generate_stream_chat(chat_json):
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(OLLAMA_URL + "/api/chat", json=chat_json) as response:
...
async def websocket_generate_stream(websocket: WebSocket):
    ...
    while True:  # Start a loop to keep the connection alive.
        try:
            ...
            chat_json = json.loads(json_str)
            print(chat_json)

            full_text = ""
            async for chunk in generate_stream_chat(chat_json):
                # print(chunk["message"])
...
# print(BASE_DIR / "frontend/out/_next")
# templates = Jinja2Templates(directory="./frontend/out")
# templates = Jinja2Templates(directory=BASE_DIR / "frontend/out")

# Mounts the `static` folder within the `build` folder to the `/static` route.
# app.mount('/static/next', StaticFiles(directory="./frontend/out/_next/static"), 'static')

# app.mount('/static/next', StaticFiles(directory=BASE_DIR / "frontend/out/_next/static"), 'static')
